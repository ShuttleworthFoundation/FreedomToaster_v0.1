...
def processElementTag(node, replacements, restart = 0):
    ...
    if node.type == 'element':
        ...
        submsgs = []
        # get or set attribute value when applicable
        if node.properties:
            for p in node.properties:
        ...
        child = node.children
        # cycle through node children to get more text content
        while child:
...
def processAttribute(node, attr):
    if attr:
        ...
        if mode=='merge':
            ...
            replaceNodeContentsWithText(attr, translation) 
        else:
            msg.outputMessage(outtxt,node.lineNo(),"", 0,node.name +":"+attr.name)
